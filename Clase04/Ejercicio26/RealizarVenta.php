...
try
{
    $listaDeProductos = Producto::LeeProductosListaJSON($_FILES["archivoProductos"]["name"]);
    $listaDeVentas =  Venta::LeerVentasListaJSON($_FILES["archivoVentas"]["name"]);
...
if(count($listaDeVentas)>0)
{
    ...
    {
        $venta->MostrarInformacion($listaDeProductos,$venta);
        echo "\n";
    }
}
echo "---------------------\n";
...
$nuevaVenta = new Venta($_POST["usuario"],$_POST["codigoProducto"],$_POST["cantidad"]);

$nuevaVenta->ConfirmarVenta($listaDeProductos,$listaDeUsuarios,$_POST["codigoProducto"],$_POST["usuario"]);

$nuevaVenta->MostrarInformacion($listaDeProductos,$nuevaVenta);
